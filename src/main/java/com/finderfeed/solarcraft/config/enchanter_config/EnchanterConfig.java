...
public class EnchanterConfig {
    ...
    public EnchanterConfig(JsonObject file){
        ...
        if (mode != null){
            ...
            SolarCraft.LOGGER.log(Level.ERROR,"Incorrect mode value in enchanter config, proceeding with static mode.");
        ...
        try {
            for (JsonElement element : file.getAsJsonArray(ENCHANTMENTS)) {
                ...
                if (cost.getSetTypes().isEmpty()){
                    SolarCraft.LOGGER.log(Level.ERROR,"Enchantment " + enchantmentID + " doesn't have runic energy baseCost. Skipping...");
                }
            ...
            SolarCraft.LOGGER.log(Level.ERROR,"Error reading enchantments array in enchanter config");
            e.printStackTrace();
        }
