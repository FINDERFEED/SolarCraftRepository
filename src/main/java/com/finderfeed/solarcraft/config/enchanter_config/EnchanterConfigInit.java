...
public class EnchanterConfigInit {
    ...
    private static boolean checkVersionAndApplyChanges(JsonObject object){
        ...
        if (version < SolarCraft.ENCHANTER_CONFIG_VERSION) {
            changesWereMade = true;
            SolarCraft.LOGGER.log(Level.INFO,"Detected outdated enchanter config. Applying changes using default values.");
            for (int i = version+1; i <= SolarCraft.ENCHANTER_CONFIG_VERSION; i++) {
                EnchanterConfigChanges.CHANGES_IN_ORDER[i].applyChanges(object);
            }
            object.remove(EnchanterConfig.VERSION);
            object.addProperty(EnchanterConfig.VERSION, SolarCraft.ENCHANTER_CONFIG_VERSION);
        }
    ...
    public static void setupJSON(){
        SolarCraft.LOGGER.log(Level.INFO,"Creating enchanter config");
        ...
        try {
            if (!Files.exists(filePath)){
                ...
                JsonObject obj = new EnchanterConfig.JsonBuilder()
                        ...
                        .setVersion(SolarCraft.ENCHANTER_CONFIG_VERSION)
                        .beginEnchantments()
            ...
            SolarCraft.LOGGER.log(Level.INFO,"Enchanter config created");
        }catch (IOException e){
            e.printStackTrace();
        }
    }
    ...
    public static void readJson(){
        SolarCraft.LOGGER.log(Level.INFO,"Reading enchanter config");
        Path filePath = configDir.resolve("enchanter_enchantments.json");
        if (Files.exists(filePath)){
            try {
                ...
                SolarCraft.LOGGER.log(Level.INFO,"Enchanter config reading complete");
            }catch (IOException e){
                SolarCraft.LOGGER.log(Level.ERROR,"Error while reading enchanter config");
                e.printStackTrace();

            }
        }else{
            SolarCraft.LOGGER.log(Level.ERROR,"Error while reading enchanter config, file doesn't exist");
