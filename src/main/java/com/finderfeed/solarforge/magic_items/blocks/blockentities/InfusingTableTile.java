    ...
    public void clientTick(){
        if (isRecipeInProgress() && (level.getGameTime() % 2 == 0) ){
            ...
            for (int i = 0; i <= 1;i++){
                ...
                ClientHelpers.ParticleConstructor c = new ClientHelpers.ParticleConstructor(ParticlesList.SMALL_SOLAR_STRIKE_PARTICLE.get(),pos.x+x,pos.y+0.9,pos.z+z,0,0,0);
            }
        }
        if (remainingRecipeTime != -1){
            Vec3 vec3 = Helpers.randomVector();
            Vec3 pos = Helpers.getBlockCenter(worldPosition);
            ClientHelpers.ParticleConstructor c = new ClientHelpers.ParticleConstructor(ParticlesList.SPARK_PARTICLE.get(),pos.x,pos.y+0.9,pos.z,vec3.x*0.01,vec3.y*0.01,vec3.z*0.01)
